...
def run_nmap_script(ip_address: str, port: int, script_name: str, xml_output_file: str) -> Optional[str]:
    ...
    try:
        ...
        # Get the current working directory and locate the scripts directory
        base_dir = os.getcwd()
        script_path = None
        
        # Traverse up to find the 'scripts' directory relative to the current working directory
        logger.info(f"Starting to search for 'scripts' directory from: {base_dir}")
        for root, dirs, files in os.walk(base_dir):
            if 'scripts' in dirs:
                potential_path = os.path.join(root, 'scripts', f"{script_name}.nse")
                logger.info(f"Found 'scripts' directory at: {root}, checking for script: {potential_path}")
                if os.path.exists(potential_path):
                    script_path = potential_path
                    break

        # Log the found script path or error if not found
        if script_path:
            logger.info(f"Using script at: {script_path}")
        else:
            raise FileNotFoundError(f"Nmap script '{script_name}' not found in any 'scripts' directory under {base_dir}.")
